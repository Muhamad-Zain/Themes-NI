...
export default function Page7({id}) {
    ...
    const handleSend = (e) => {
        e.preventDefault()
        const messages = ref(database, `weddings-NI-1/${id}/expresion/${Date.now()}`)
        set(messages, {
            name,
            text: message
        })
    ...
    useEffect(() => {
        const messagesRef = ref(database, `weddings-NI-1/${id}/expresion`); // referensi untuk mengambil ucapan
